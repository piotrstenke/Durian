...
namespace Durian.Manager
{
	...
	public sealed class DisabledModuleAnalyzer : DiagnosticAnalyzer, ISourceGenerator
	{
		...
		/// </summary>
		/// <param name="module"><see cref="DurianModule"/> to check if is enabled.</param>
		/// <exception cref="InvalidOperationException">Analyzer is not initialized.</exception>
		...
		/// <summary>
		/// Checks if the specified <paramref name="module"/> is enabled for the current <paramref name="compilation"/>.
		/// </summary>
		/// <param name="module"><see cref="DurianModule"/> to check if is enabled.</param>
		/// <param name="compilation"><see cref="Compilation"/> that is used if the analyzer is not initialized or the disabled managers are changed.</param>
		...
		/// <summary>
		/// Checks if all the specified <paramref name="modules"/> are enabled for the current compilation.
		/// </summary>
		/// <param name="modules">A collection of <see cref="DurianModule"/>s to check if are enabled.</param>
		/// <exception cref="InvalidOperationException">Analyzer is not initialized.</exception>
		public static bool AreEnabled(DurianModule[]? modules)
		{
			...
			lock (_lockObject)
			{
				...
				return AreEnabled_Internal(modules);
			}
		}

		/// <summary>
		/// Checks if all the specified <paramref name="modules"/> are enabled for the current <paramref name="compilation"/>
		/// </summary>
		/// <param name="modules">A collection of <see cref="DurianModule"/>s to check if are enabled.</param>
		/// <param name="compilation"><see cref="Compilation"/> that is used if the analyzer is not initialized or the disabled managers are changed.</param>
		/// <exception cref="ArgumentNullException"><paramref name="compilation"/> is <see langword="null"/>.</exception>
		public static bool AreEnabled(DurianModule[]? modules, Compilation compilation)
		{
			if (compilation is null)
			{
				throw new ArgumentNullException(nameof(compilation));
...
#pragma warning restore RS1012 // Start action has no registered actions.
		...
		private static bool AreEnabled_Internal(DurianModule[] modules)
		{
			foreach (DurianModule module in modules)
			{
				if (!IsEnabled_Internal(module))
