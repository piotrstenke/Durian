...
public sealed class TestableCompilationData : ICompilationData
{
	...
	/// <summary>
	/// A <see cref="CSharpCompilation"/> that is affected by the <see cref="UpdateCompilation(SyntaxTree)"/> method or its overloads.
	/// </summary>
	...
	/// <summary>
	/// Original <see cref="CSharpCompilation"/> that is not affected by the <see cref="UpdateCompilation(SyntaxTree)"/> method or its overloads.
	/// </summary>
	...
	/// Creates a new <see cref="TestableCompilationData"/>.
	/// </summary>
	/// <param name="sources">A collection of <see cref="string"/>s that will be parsed as <see cref="SyntaxTree"/>s and added to the newly-created <see cref="TestableCompilationData"/>.</param>
	...
	/// Creates a new <see cref="TestableCompilationData"/>.
	/// </summary>
	/// <param name="source">A <see cref="string"/> that will be parsed as a <see cref="SyntaxTree"/> and added to the newly-created <see cref="TestableCompilationData"/>.</param>
	...
	/// <param name="index">Index at which the <see cref="IMemberData"/> should be returned. Can be thought of as a number of <see cref="SyntaxNode"/> of type <typeparamref name="TNode"/> to skip before creating a valid <see cref="IMemberData"/>.</param>
	/// <typeparam name="TNode">Type of <see cref="SyntaxNode"/> to find and convert to a <see cref="IMemberData"/>.</typeparam>
	/// <returns>
	/// A new <see cref="IMemberData"/> created from a <see cref="SyntaxTree"/> of type <typeparamref name="TNode"/> found at the specified index in the parsed <see cref="SyntaxTree"/> -or-
	...
	/// <param name="index">Index at which the <see cref="SyntaxNode"/> should be returned. Can be thought of as a number of <see cref="SyntaxNode"/> of type <typeparamref name="TNode"/> to skip.</param>
	...
	/// <see langword="null"/> if no such <see cref="SyntaxNode"/> exists.
	/// </returns>
	public TNode? GetNode<TNode>(string? source, int index = 0) where TNode : SyntaxNode
	{
		if (source is null)
		{
			return null;
		...
		return GetNode<TNode>(CSharpSyntaxTree.ParseText(source, encoding: Encoding.UTF8), index);
	}

	/// <summary>
	/// Returns the <see cref="SyntaxNode"/> of type <typeparamref name="TNode"/> at the specified index in the <paramref name="syntaxTree"/>.
	/// </summary>
	/// <param name="syntaxTree">A <see cref="SyntaxTree"/> to get the <see cref="SyntaxNode"/> of type <typeparamref name="TNode"/> from.</param>
	/// <param name="index">Index at which the <see cref="SyntaxNode"/> should be returned. Can be thought of as a number of <see cref="SyntaxNode"/> of type <typeparamref name="TNode"/> to skip.</param>
	/// <typeparam name="TNode">Type of <see cref="SyntaxNode"/> to find and return.</typeparam>
	/// <returns>
	...
	/// <see langword="null"/> if no such <see cref="SyntaxNode"/> exists.
	/// </returns>
	public TNode? GetNode<TNode>(SyntaxTree? syntaxTree, int index = 0) where TNode : SyntaxNode
	{
	...
	/// <see langword="null"/> if no such <see cref="SyntaxNode"/> exists.
	/// </returns>
	public TSymbol? GetSymbol<TSymbol>(SyntaxNode? node) where TSymbol : class, ISymbol
	{
		if (node is null)
		{
			return null;
	...
	/// </summary>
	/// <param name="syntaxTree"><see cref="SyntaxTree"/> to retrieve the <see cref="ISymbol"/> from.</param>
	/// <param name="index">Index at which the <see cref="ISymbol"/> should be returned. Can be thought of as a number of <see cref="SyntaxNode"/> of type <typeparamref name="TNode"/> to skip before retrieving a valid <see cref="ISymbol"/>.</param>
	/// <typeparam name="TSymbol">Type of <see cref="ISymbol"/> to retrieve from the specified <paramref name="syntaxTree"/>.</typeparam>
	/// <typeparam name="TNode">Type of <see cref="SyntaxNode"/> to find and convert to a <see cref="ISymbol"/> of type <typeparamref name="TSymbol"/>.</typeparam>
	/// <returns>
	/// An <see cref="ISymbol"/> of type <typeparamref name="TSymbol"/> retrieved from the target <paramref name="syntaxTree"/> -or-
	/// <see langword="null"/> if no such <see cref="SyntaxNode"/> exists.
	/// </returns>
	public TSymbol? GetSymbol<TSymbol, TNode>(SyntaxTree? syntaxTree, int index = 0) where TSymbol : class, ISymbol where TNode : SyntaxNode
	{
	...
	/// <typeparam name="TNode">Type of <see cref="SyntaxNode"/> to find and convert to a <see cref="ISymbol"/> of type <typeparamref name="TSymbol"/>.</typeparam>
	/// <returns>
	/// An <see cref="ISymbol"/> of type <typeparamref name="TSymbol"/> retrieved from a <see cref="SyntaxTree"/> of type <typeparamref name="TNode"/> found at the specified index in the parsed <see cref="SyntaxTree"/> -or-
	/// <see langword="null"/> if no such <see cref="SyntaxNode"/> exists.
	/// </returns>
	public TSymbol? GetSymbol<TSymbol, TNode>(string? source, int index = 0) where TSymbol : class, ISymbol where TNode : SyntaxNode
	{
	...
	/// <summary>
	/// Parses the <paramref name="source"/> <see cref="string"/> and adds the created <see cref="SyntaxTree"/> to the <see cref="Compilation"/>.
	/// </summary>
	...
	public void UpdateCompilation(string? source)
	{
		...
		UpdateCompilation(CSharpSyntaxTree.ParseText(source, encoding: Encoding.UTF8));
	}
