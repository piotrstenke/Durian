...
public abstract class SyntaxValidator<TContext> : GeneratorSyntaxFilter, ISyntaxValidatorWithDiagnostics<TContext> where TContext : ISyntaxValidationContext
{
	...
	/// <inheritdoc/>
	public sealed override IEnumerable<IMemberData> Filter(
	...
	)
	{
		...
		return Filter(compilation, list, diagnosticReceiver, cancellationToken);
	}
	...
	)
	{
		...
		return Filter(compilation, list, cancellationToken);
	}
	...
	/// <inheritdoc/>
	public override IEnumerable<IMemberData> Filter(IGeneratorPassContext context)
	{
		...
		{
			return Filter(context.TargetCompilation, list, context.CancellationToken);
		}

		if (diagnosticReceiver is not INodeDiagnosticReceiver node)
		{
			return Filter(context.TargetCompilation, list, diagnosticReceiver, context.CancellationToken);
		}
