...
namespace Durian.Analysis.Filtration
{
	...
	public interface ISyntaxFilterWithDiagnostics : ISyntaxFilter
	{
		...
		/// <param name="diagnosticReceiver"><see cref="IDiagnosticReceiver"/> to report the diagnostics to.</param>
		/// <param name="cancellationToken"><see cref="CancellationToken"/> that specifies if the operation should be canceled.</param>
		IEnumerable<IMemberData> Filtrate(ICompilationData compilation, IEnumerable<SyntaxNode> collectedNodes, IDiagnosticReceiver diagnosticReceiver, CancellationToken cancellationToken = default);
	}
}
