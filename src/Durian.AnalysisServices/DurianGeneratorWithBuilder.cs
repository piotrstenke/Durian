...
/// <summary>
/// A <see cref="DurianSourceGenerator"/> that uses a <see cref="CodeBuilder"/> to generate code.
/// </summary>
/// <typeparam name="TContext">Type of <see cref="IGeneratorPassContext"/> this generator uses.</typeparam>
public abstract class DurianGeneratorWithBuilder<TContext> : DurianSourceGenerator<TContext> where TContext : GeneratorPassBuilderContext
{
	...
	/// <param name="context"><typeparamref name="TContext"/> to add the source to.</param>
	protected void AddSourceWithOriginal(string hintName, SyntaxNode original, TContext context)
	{
		SyntaxTree tree = ParseSyntaxTree(context);
		context.CodeBuilder.Clear();
	...
	protected virtual string SymbolToString(ISymbol symbol)
	{
		...
		switch (symbol)
		{
			...
			case IMethodSymbol method when method.MethodKind == MethodKind.StaticConstructor:
				...
				return builder + "static " + method.ContainingType.Name + "()";
