...
namespace Durian
{
	...
	public sealed class CodeBuilder
	{
		...
		/// <summary>
		/// Writes the file header returned by the <see cref="AutoGenerated.GetHeader(string?, string?)"/> method using values provided by the <see cref="SourceGenerator"/>.
		/// </summary>
		/// <remarks>If the <see cref="SourceGenerator"/> is <see langword="null"/>, calls the <see cref="AutoGenerated.GetHeader()"/> method instead.</remarks>
		public void WriteHeader()
		{
			...
			{
				text = AutoGenerated.GetHeader(SourceGenerator.GeneratorName, SourceGenerator.Version);
			}
		...
		/// Is the <see cref="SourceGenerator"/> property is <see langword="null"/>,
		...
		public void WriteUsings(SemanticModel semanticModel, CSharpSyntaxNode node, CancellationToken cancellationToken = default)
		{
			if (SourceGenerator is null)
			{
				throw new InvalidOperationException($"If the {nameof(SourceGenerator)} property is null, a {nameof(INamespaceSymbol)} of the assembly's global namespace must be explicitly provided using one of WriteUsings 3-parameter overloads.");
			}

			WriteUsings_Internal(semanticModel.GetUsedNamespaces(node, SourceGenerator.TargetCompilation, true, cancellationToken));
		}
		...
		{
			return ParseSyntaxTree(SourceGenerator?.ParseOptions);
		}
