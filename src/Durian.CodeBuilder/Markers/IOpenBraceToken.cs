...
{
	/// <summary>
	/// Provides a method for writing the open brace '{' token.
	/// </summary>
	/// <typeparam name="TReturn">Type of returned builder.</typeparam>
	public interface IOpenBraceToken<out TReturn> : IWhiteSpace where TReturn : class
	{
	}
}
...
namespace Durian.Builder
{
	...
	{
		internal partial class Adapter : IOpenBraceToken<IWhiteSpace>
		{
		}
	}
	...
	{
		/// <summary>
		/// Writes an open brace '{' token.
		/// </summary>
		/// <typeparam name="T">Type of target builder.</typeparam>
		/// <param name="builder">Code builder to write to.</param>
		public static T OpenBrace<T>(this IOpenBraceToken<T> builder) where T : class
		{
