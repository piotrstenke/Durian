...
{
	/// <summary>
	/// Provides a method for writing the greater than '&gt;' token.
	/// </summary>
	/// <typeparam name="TReturn">Type of returned builder.</typeparam>
	public interface IGreaterThanToken<out TReturn> : IWhiteSpace where TReturn : class
	{
	}
}
...
namespace Durian.Builder
{
	...
	{
		internal partial class Adapter : IGreaterThanToken<IWhiteSpace>
		{
		}
	}
	...
	{
		/// <summary>
		/// Writes a greater than '&gt;' token.
		/// </summary>
		/// <typeparam name="T">Type of target builder.</typeparam>
		/// <param name="builder">Code builder to write to.</param>
		public static T GreaterThan<T>(this IGreaterThanToken<T> builder) where T : class
		{
