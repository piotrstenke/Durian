...
{
	/// <summary>
	/// Provides a method for writing the <see langword="static"/> keyword.
	/// </summary>
	/// <typeparam name="TReturn">Type of returned builder.</typeparam>
	public interface IStaticKeyword<out TReturn> : IWhiteSpace where TReturn : class
	{
	}
}
...
namespace Durian.Builder
{
	...
	{
		internal partial class Adapter : IStaticKeyword<IWhiteSpace>
		{
		}
	}
	...
	public static partial class RawCodeBuilderExtensions
	{
		...
		public static T Static<T>(this IStaticKeyword<T> builder) where T : class
		{
			ICodeBuilder code = (builder as ICodeBuilder)!;
			code.CodeReceiver.Write("static");
			return (code as T)!;
		}
	}
}
