...
{
	/// <summary>
	/// Provides a method for writing the close bracket ']' token.
	/// </summary>
	/// <typeparam name="TReturn">Type of returned builder.</typeparam>
	public interface ICloseBracketToken<out TReturn> : IWhiteSpace where TReturn : class
	{
	}
}
...
namespace Durian.Builder
{
	...
	{
		internal partial class Adapter : ICloseBracketToken<IWhiteSpace>
		{
		}
	}
	...
	{
		/// <summary>
		/// Writes a close bracket ']' token,
		/// </summary>
		/// <typeparam name="T">Type of target builder.</typeparam>
		/// <param name="builder">Code builder to write to.</param>
		public static T CloseBracket<T>(this ICloseBracketToken<T> builder) where T : class
		{
