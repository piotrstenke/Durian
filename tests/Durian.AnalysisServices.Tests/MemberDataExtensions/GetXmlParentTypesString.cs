...
namespace Durian.Analysis.Tests.MemberDataExtensions
{
	public sealed class GetXmlParentTypesString : CompilationTest
	{
		...
		{
			Assert.True(GetMethod("class Test { void Method<T>(int a) { } }")!.GetXmlParentTypesString(true, true) == "Test.Method{T}(int)");
		}

		[Fact]
		public void ReturnsNameWithParameters_When_IsGenericMethod_And_ParentIsGeneric_And_IncludeParametersIsTrue()
		{
			Assert.True(GetMethod("class Test<T> { void Method<U>(U a, int b) { } }")!.GetXmlParentTypesString(true, true) == "Test{T}.Method{U}(U, int)");
		}

		[Fact]
		public void ReturnsNameWithParameters_When_IsMethod_And_IncludeParametersIsTrue()
		{
			Assert.True(GetMethod("class Test { void Method(int a) { } }")!.GetXmlParentTypesString(true, true) == "Test.Method(int)");
		}
