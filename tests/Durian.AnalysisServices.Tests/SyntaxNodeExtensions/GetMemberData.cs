...
namespace Durian.Analysis.Tests.SyntaxNodeExtensions
{
	public sealed class GetMemberData : CompilationTest
	{
		...
		public void ReturnsValidEventData_When_HasMultipleEventFieldsInSingleDeclaration()
		{
			...
			Data.EventData? data = e.GetMemberData(Compilation) as Data.EventData;
		...
		{
			Assert.True(ValidateMember<Data.MemberData, MemberDeclarationSyntax>("class Test { int this[int index] { get => 0 }}"));
		}

		[Fact]
		public void ReturnsValidMethodData_When_IsMethod()
		{
			Assert.True(ValidateMember<Data.MethodData, MethodDeclarationSyntax>("class Test { void Method() { } }"));
		}
