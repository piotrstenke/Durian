...
namespace Durian.Tests.AnalysisCore.AnalysisUtilities
{
	public sealed class KeywordToType
	{
		...
		{
			Assert.True(Durian.AnalysisUtilities.KeywordToType(null) == string.Empty);
		}
		...
		{
			Assert.True(Durian.AnalysisUtilities.KeywordToType(keyword) == expected);
		}

		[Fact]
		public void ReturnsInput_When_InputIsNotCSharpTypeKeyword()
		{
			Assert.True(Durian.AnalysisUtilities.KeywordToType("type") == "type");
		}
